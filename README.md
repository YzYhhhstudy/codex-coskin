...
| 平台 | 双击它 | 做什么 |
| --- | --- | --- |
| macOS | `双击换肤.command` | 启动 Codex（带皮肤）+ 拉最新 + 重上你上次用过的主题；第一次还会装好 Codex 技能 |
| Windows | `双击换肤.bat` | 同上 |
...
- **Windows —— 没有 `.app` 这种东西**：Windows 不存在 `.app` 格式。等价做法是给 `双击换肤.bat`
  **建个快捷方式**（右键 → 发送到 → 桌面 / 或直接创建快捷方式）→ 快捷方式「属性」里把「运行方式」设为**最小化**
  → 再**钉到任务栏**。这样点任务栏图标就等于运行它，控制台只在任务栏一闪。
  （命令行本身已支持 `--gui`：确认/报错走系统弹窗；将来若做真正的无窗口 Windows 启动器就用它。目前 Windows 端未在真机充分验证。）
