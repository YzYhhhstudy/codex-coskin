...
`npm test`（= `node test/run-mock.mjs`）一条命令跑完两套，**153 条断言**，零参数零依赖。
单跑用 `npm run test:core` / `npm run test:home`。
runner 自己起静态服务 + 无头 Chrome（临时 profile、独立端口 9666/9667），跑完自动清理，
**绝不碰使用者正在跑的 Codex**（那边是 9341）。

- **core（`test/mock/core.html`，53 条）**：平台无关的核心机制——
  token 接管 / 面板挂载 / 配色编辑器 / 桌宠(开关·拖拽·缩放夹子) / 面板缩放 /
  板子质感四档 / 可见度档位 / 还原逐项回基线 / 闭包清理 / 代际门。
- **home（`test/mock/home.html`，100 条）**：首页装饰——历次真机翻车全在这一段——
  板子几何(板顶=工作区上界+50、包住图标与标题、22%~55% 夹子) / 内容分块上移(不累加、不拽走输入框) /
  标题结构优先识别 / 行书字按原文认卡 + 二级条目 / Codex↔ChatGPT 模式判别 /
  品牌行按实测顶栏控件让位(含收侧栏) / 原图档零遮挡 / 缩窗重贴合 / 离开首页与还原的「原样奉还」。

- **`test/mock-verify.mjs`** 是更早的整页回归，依赖一份早已丢失的临时目录 mock，已被上面两套取代，留作参考。

### 第三层：`npm run smoke`（真机烟雾测试，只读）

**mock 只能锁住「我们对真机的假设」，锁不住「真机变了」。** Codex 升级改了类名或结构，
两套 mock 照样全绿、真机却崩——这是任何 mock 的天花板，不是做法问题。
`test/smoke-live.mjs` 连上**正在跑的 Codex**，只读核对 7 组契约：结构锚点 / 首页形状 /
模式判别标签 / token 变量族 / 主题 match 还能不能命中真机卡片原文 / 品牌行让位 / 板子几何。
全程只有 `querySelector` / `getComputedStyle` / `getBoundingClientRect`——不换肤、不改样式、
不动焦点、不重启，使用者可以一边用一边跑。看不到的部分（非首页、未应用皮肤）报「跳过」而不是失败。

首次真机运行（2026-07-23，Codex 首页 + xiuxing 系皮肤）：**25 项全绿**。
顺带证伪了一个担心：「Codex」产品名标签本身在可点控件里，已参与品牌行让位测量，不会被压住。

### 第四层：换肤时的启动自检（`CONTRACT_SCRIPT`）

烟雾测试有个前提——**得有人记得跑**。用户不会跑。
所以把它最要命的那几条做成 `CONTRACT_SCRIPT`，在 `applyTheme` 末尾只读跑一次：
正常时一声不吭，结构对不上就当场说人话（哪块失效 + 什么后果 + 怎么修），GUI 模式弹窗。

三条设计原则，改的时候别破坏：
1. **永远不影响换肤结果**——自检自己抛错就 `catch` 掉当没发生。皮肤已经上了，不能因为自检失败回滚。
2. **不在首页时绝不报首页项**——用户在会话里换肤收到「首页卡片按钮缺失」是纯噪音，
   一次假警报就会让人以后忽略所有告警。
3. **只报「缺了就会明显坏」的**，完整体检留给 `npm run smoke`。告警多 = 告警等于没有。

### 写 mock 页时最容易骗自己的三件事

1. **结构要照抄真机的「形状」，不是照抄外观。** 首页标题与卡片在**不同子树**（锚容器里没有标题），
   注入代码正是据此把两块分别上移。图省事把标题塞进锚容器，上移会叠加两次，几何断言全是假的。
2. **flex 项的 `min-width: auto` 会被溢出内容撑大。** 卡片行加宽后横向溢出，把主区宽度顶出去，
   按主区算的板宽/紧凑宽全跟着漂——mock 里要 `min-width: 0` 压住（真机主区是定宽的）。
3. **描边要从几何断言里扣掉。** 板子有 1px 描边，外框比代码设定的 width/height 各多 2px。
   夹子明明精确生效，不扣就会报 55.1% > 55% 这种假失败。
