...
> **好消息（2026-07-23 审计）**：v0.29~v0.34 这一大批界面改动（标题板几何、标题提层、板子质感四档、
> Codex/ChatGPT 模式判别、卡片样式作用域、品牌行避让）**全部在注入层**，跑在 Codex 自己的 Chromium 里，
> **Windows 无需任何适配即可生效**。已顺带修掉三处"伪平台无关"的坑：行书字体栈补 Windows 楷体、
> 品牌行避让改为实测控件边缘（Windows 窗口按钮在右侧）、PowerShell 弹窗加 TopMost owner。
> 所以下面这份清单只剩**宿主层**要真机验：
